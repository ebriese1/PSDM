...
% % % Do not need MW or VB for inorganic compounds (edit 1/2/2023)
...
WT = input(15,1); % WT: mass of adsorbent in column, g
...
KF = input(27,1:NCOMP); % Film transfer coefficient, cm/s
DS = input(28,1:NCOMP); % Surface diffusivity,cm^2/s
DP = input(29,1:NCOMP);% Pore diffusivity based on pore void fraction,cm^2/s
DL = input(35,1:NCOMP); % Liquid diffusivity, cm^2/s
SC = input(36,1:NCOMP); % Schimit number, dim.
DTOL = input(32,1); % total run time, min.
...
for i = 1:NCOMP
    ...
    % correlation, cm^2/s
    % Correlation only applicable to ORGANIC COMPOUNDS
%     if DL(i)>=0
%     else
...
ParamTable = [XK,XN,RE,SC,SF,SFR,EBED,WT,BEDVOL,EPOR,KF,DL,DP,DS,VISC,SPDFR,EBCT,RAD,BIS,BIP,FLRT]; % Additional Outputs 
