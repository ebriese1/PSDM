...
% Only applicable for organic compounds(Edit out 1/26/2023) 
...
ParamTable = [XK,XN,RE,SC,SF,SFR,EBED,WT,BEDVOL,EPOR,KF,DL,DP,DS,VISC,SPDFR,EBCT,RAD,BIS,BIP]; % Additional Outputs to check things
...
%  options = odeset('RelTol',1e-4,'AbsTol',1e-6); % Error with relative tolerances: Use a way to set tolerances to a higher value 
...
%% Plot results
for i=1:NCOMP
ind(i) = find(breakthrough(:,i) < 0, 1, 'last');
x{i} = BV(ind(i)+1:end);
y{i} = breakthrough(ind(i)+1:end,i);
end


plot(x{1}, y{1}, 'linewidth', 1.5)
hold on
plot(x{2}, y{2}, 'linewidth', 1.5)
hold off
